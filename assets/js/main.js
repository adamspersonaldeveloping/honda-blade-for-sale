...
(function ($) {
  ...
  $(".gallery.lightbox")
    .on("click", "a", function (event) {
      ...
      // Not an image? Bail.
      if (!href.match(/\.(jpg|gif|png|mp4|webp)$/)) return;
  ...
  // Smart Autoplay/Pause on Scroll using Intersection Observer
  document.addEventListener("DOMContentLoaded", function () {
    ...
    var observerOptions = {
      ...
      rootMargin: "0px",
      threshold: 0.6, // Video must be 60% visible on screen to trigger play
    };
      ...
      entries.forEach(function (entry) {
        ...
        if (entry.isIntersecting) {
          ...
          video.play().catch(function (error) {
            // Catch block handles any browser-specific restrictions gracefully
            console.log(
              "Autoplay prevented: user interaction required for sound."
            );
          });
        } else {
    ...
    // Bind observer to each video
    videos.forEach(function (video) {
      observer.observe(video);
    });
  });

  // Automatically pause other videos when a new one starts playing
